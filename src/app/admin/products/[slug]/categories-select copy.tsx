...
/**
 * Props for CategoriesSelect component
 */
interface CategoriesSelectProps
  ...
  options: {
    ...
    value: string;
    /** Optional array of options to be displayed as a dropdown. */
    subCategories?: {
      label: string;
      value: string;
    }[];
...
export const CategoriesSelect = React.forwardRef<
  HTMLButtonElement,
    ...
    const toggleAll = () => {
      if (
        ...
        options.flatMap((option) =>
          option.subCategories?.map((sub) => sub.value)
        ).length
      ) {
        handleClear();
      } else {
        const allSubCategoryValues = options.flatMap(
          (option) =>
            option.subCategories?.map((sub) => sub.value) ||
            []
        );
        setSelectedValues(allSubCategoryValues);
        onValueChange(allSubCategoryValues);
      }
    };
