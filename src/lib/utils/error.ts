...
export function parseZodError(error: ZodError): AppError {
  ...
  return createError("VALIDATION_ERROR", error.message, 400, context);
}

export function parseDrizzleError(error: DrizzleError): AppError {
  return createError("DATABASE_ERROR", error.message, 500);
}

export function parseBetterAuthError(error: APIError): AppError {
  return createError(
    "AUTH_ERROR",
    error.message,
