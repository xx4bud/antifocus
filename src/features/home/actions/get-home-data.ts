...
export async function getHomeCategories(limit = 12) {
  return db
    .select({
...
export async function getHomeFeaturedProducts(limit = 12) {
...
export async function resolveSlug(slug: string) {
  const [cat] = await db
    ...
    .from(schema.categories)
    .where(
      and(eq(schema.categories.slug, slug), eq(schema.categories.enabled, true))
    )
    .limit(1);
  if (cat) {
...
export async function getProductBySlug(slug: string) {
  ...
  const [variants, mediaRows, cats, orgs] = await Promise.all([
    db
      .select()
...
export async function getUserByUsername(username: string) {
  ...
  const orgs = await db
    ...
    .where(
      and(eq(schema.members.userId, user.id), eq(schema.members.enabled, true))
    );
...
export async function searchProducts(params: {
  ...
  const { query, limit = 20, offset = 0, sortBy = "newest" } = params;
  ...
  const [data, totalResult] = await Promise.all([
    db
      ...
      .limit(limit)
      .offset(offset),
    db.select({ count: count() }).from(schema.products).where(conditions),
  ]);
